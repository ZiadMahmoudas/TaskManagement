...
@Component({
  ...
  template: `
    ...
    >
      ...
      >
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-purple-400 resize-none"
            ></textarea>
          </div>
          <div>
        ...
        <div class="flex gap-3 mt-6">
          <button
            (click)="closeCreate()"
            class="flex-1 px-4 py-2 border border-gray-200 text-gray-500 text-sm rounded-lg hover:bg-gray-50"
          >
            Cancel
          </button>
          <button
    ...
    >
      <div
        class="bg-white rounded-2xl p-8 w-full max-w-sm shadow-2xl text-center"
        (click)="$event.stopPropagation()"
      >
...
export class ProjectsComponent implements OnInit {
 ...
 submitCreate() {
  ...
  submitEdit() {
    ...
    this.formLoading = true;
    this.http
      .put<Project>(`${this.API}/${this.selectedProject.projectId}`, this.editForm, this.headers())
      .subscribe({
        next: (updated) => {
        ...
        error: () => {
          this.formError = 'Failed to update project.';
          this.formLoading = false;
          this.cdr.markForCheck();
        },
      });
